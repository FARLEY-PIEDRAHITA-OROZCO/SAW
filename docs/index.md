---
layout: home

hero:
  name: "SAW"
  text: "Seguridad de Aplicaciones Web"
  tagline: Cuaderno digital estructurado del técnico SENA en Seguridad de Aplicaciones Web. Todo el contenido del programa organizado por fases, temas y habilidades transversales.
  image:
    src: /favicon.svg
    alt: SAW Wiki
  actions:
    - theme: brand
      text: Comenzar
      link: /fase1/metodologias-normas-estandares/introduccion
    - theme: alt
      text: Ver la Fase 1
      link: /fase1/

features:
  - icon: "🛡️"
    title: "Fase 1 · Análisis"
    details: Fundamentos de seguridad, infraestructura web, normatividad y estándares, metodologías, vulnerabilidades, pentesting y OWASP Top 10.
    link: /fase1/
    linkText: Explorar
  - icon: "🗣️"
    title: "Comunicación oral y escrita"
    details: "Argumentación, comunicación asertiva, producción de textos y relacionamiento: las competencias comunicativas del programa."
    link: /fase1/argumentacion/
    linkText: Explorar
  - icon: "💻"
    title: "TIC"
    details: Software, internet, ofimática, evaluación y mejora continua como soporte tecnológico del proceso formativo.
    link: /fase1/tic/
    linkText: Explorar
  - icon: "📖"
    title: "Glosario"
    details: Términos y conceptos clave de la seguridad de aplicaciones web, siempre a mano.
    link: /glosario
    linkText: Consultar
  - icon: "🔗"
    title: "Referencias"
    details: OWASP, WSTG, ISO 27001, Guías del MinTIC y más fuentes oficiales con enlace directo.
    link: /referencias
    linkText: Consultar
  - icon: "🔍"
    title: "Búsqueda integrada"
    details: Buscador local sobre todo el contenido para encontrar al instante conceptos, normas y herramientas.
---

## Estructura del contenido

El wiki se organiza siguiendo las **fases del programa** del SENA (Análisis, Diseño, Implementación y Evaluación). Cada fase reúne **OVAs independientes**: cada una es una carpeta modular que va de la introducción hasta su desarrollo y síntesis.

```text
SAW Wiki/
├─ Fase 1 · Análisis
   ├─ Metodologías, normas y estándares
   │   ├─ Introducción
   │   ├─ Fundamentos
   │   ├─ Infraestructura de aplicaciones web
   │   ├─ Normatividad y estándares
   │   ├─ Metodologías
   │   ├─ Gestión de vulnerabilidades
   │   ├─ Pentesting
   │   ├─ OWASP Top 10
   │   ├─ Herramientas especializadas
   │   └─ Análisis de resultados
   ├─ Comunicación · Argumentación
   ├─ Comunicación · Comunicándonos
   ├─ Comunicación · Escribiendo
   ├─ Comunicación · Relacionando
   └─ TIC
       ├─ Software · Internet · Ofimática
       └─ Evaluación · Mejora continua
```

::: tip ¿Cómo se integran las OVAs?
Cada OVA es **independiente y modular**: puedes estudiarlas en paralelo, en el orden que prefieras. La ruta técnica principal es la OVA de Metodologías, normas y estándares, mientras las transversales de **Comunicación** y **TIC** se cursan en paralelo. Cuando el SENA publique la siguiente fase, se creará su carpeta `fase2/` con los mismos criterios.
:::